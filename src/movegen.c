...
int generate_pawn_moves(struct Move* move_list, struct Position *pos) {
    ...
    const enum Side them = (us == WHITE) ? BLACK : WHITE;
    const enum Direction up = (us == WHITE) ? NORTH : SOUTH;
    const enum Direction down = (us == WHITE) ? SOUTH : NORTH;

    const u64 our_rank7bb = (us == WHITE) ? Rank7BB : Rank2BB;
    const u64 our_rank3bb = (us == WHITE) ? Rank3BB : Rank6BB;

    const u64 pawns = pos->piece_bb[PAWN][us];
    ...
    while (promotion_pawns) {
        const enum Square from = pop_lsb(&promotion_pawns);
        //Attacked squares
        u64 attacked_sqs = attack_set.pawn[us][from] & pos->occupied_squares[them];
        while (attacked_sqs) {
            const enum Square to = pop_lsb(&attacked_sqs);

            const struct Move promotion_move = create_special_move(PROMOTION, KNIGHT, from, to);
    ...
    while (promotion_targets) {
        const enum Square to = pop_lsb(&promotion_targets);
        const enum Square from  = (us == WHITE) ? to - 8 : to + 8;
    ...
    while (non_promotion_pawns) {
        const enum Square from = pop_lsb(&non_promotion_pawns);
        u64 attacked_sqs = attack_set.pawn[us][from] & pos->occupied_squares[them];

        while (attacked_sqs) {
            const enum Square to = pop_lsb(&attacked_sqs);
            const struct Move new_move = create_regular_move(from, to);
    ...
    while (single_push_targets) {
        const enum Square to = pop_lsb(&single_push_targets);
        const enum Square from = (us == WHITE) ? to - 8 : to + 8;
    ...
    while (double_push_targets) {
        const enum Square to = pop_lsb(&double_push_targets);
        const enum Square from = (us == WHITE) ? to - 16 : to + 16;
    ...
    if (pos->ep_square != SQUARE_EMPTY) {
        const u64 ep_square_bb = set_bit(pos->ep_square);
        const u64 ep_attacker_squares = shift_E(shift_bb(ep_square_bb, down)) | shift_W(shift_bb(ep_square_bb, down));
        ...
        while (ep_attackers) {
            const enum Square from = pop_lsb(&ep_attackers);
            const struct Move new_ep_move = create_special_move(ENPASSANT, PT_NULL, from, pos->ep_square);
...
int generate_moves_pt(enum Piece_type pt, struct Move *move_list, struct Position *pos) {
    ...
    while (piece_squares) {
        const enum Square from = pop_lsb(&piece_squares);
    ...
    if (pt == KING) {
        if (can_kingside_castle(us, pos)) {
            const enum Square from = (us == WHITE) ? e1 : e8;
            const enum Square to = (us == WHITE) ? g1 : g8;
            const struct Move new_move = create_special_move(CASTLING, PT_NULL, from, to);
            if (legal(new_move, pos, tmp_stack)) {
                *move_list++ = new_move;
                ++num_moves_added;
            }
        ...
        if (can_queenside_castle(us, pos)) {
            const enum Square from = (us == WHITE) ? e1 : e8;
            const enum Square to = (us == WHITE) ? c1 : c8;
            const struct Move new_move = create_special_move(CASTLING, PT_NULL, from, to);
